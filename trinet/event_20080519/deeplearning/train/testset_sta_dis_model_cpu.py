...
from dl_load_data import load_mydataset, get_rebuild_data, model_data_process, gen_sta_dis_matrix
from dl_paraconfig import cuda_visible_devices_str_para, seed_para, train_techo_file_path, test_techo_file_path, test_prediction_model_path, test_show_path, vel_base_path, disc_660, range_660, label_range_dep, label_range_vel, modify_velocity_percent, vel_model_info_dic, draw_depth, test_loss_file_path, dl_tr_data_path
...
load_dataset_test = load_mydataset(techo_file_path=test_techo_file_path)
...
testset_pre_model_dir_path = dl_tr_data_path + '/testset_rebuild_seis_data_sta_dis' + '/prediction_model'
testset_show_dir_path = dl_tr_data_path + '/testset_rebuild_seis_data_sta_dis' + '/pre_show'
testset_loss_file_dir_path = dl_tr_data_path + '/testset_rebuild_seis_data_sta_dis'


mkdir(testset_pre_model_dir_path)
mkdir(testset_show_dir_path)


test_techo_file_temp = os.listdir('../../../../data/workspace_event_20080519_data/data/data0/rebuild_seis_data_sta_dis/seis_rebuild_data')
test_techo_file = ['../../../../data/workspace_event_20080519_data/data/data0/rebuild_seis_data_sta_dis/seis_rebuild_data/'+i for i in test_techo_file_temp]
...
samp_path_temp = os.path.dirname(os.path.dirname(os.path.dirname(test_techo_file[0].replace('\n',''))))+'/rebuild_seis_data/seis_rebuild_data/'+os.path.basename(test_techo_file[0].replace('\n',''))[:-8]+'.dat'
pro_time, _, _, _ = get_rebuild_data(samp_path_temp)
...
for i in range(len(test_techo_file)):
    ...
    model_path = files_dir_path_1+'/modified_modelfile/modify_model'+techo[:-4]+'.dat'
    ...
    show_pic_dic_path = testset_show_dir_path +'/pre_model'+str(techo)
...
f = open(testset_loss_file_dir_path+'/loss.dat', 'w')
