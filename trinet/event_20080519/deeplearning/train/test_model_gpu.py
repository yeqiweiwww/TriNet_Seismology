...
from dl_paraconfig import cuda_visible_devices_str_para, seed_para, train_techo_file_path, test_techo_file_path, test_prediction_model_path, test_show_path, vel_base_path, disc_660, range_660, label_range_dep, label_range_vel, modify_velocity_percent, vel_model_info_dic, draw_depth, test_loss_file_path
...
mkdir(test_prediction_model_path)
mkdir(test_show_path)

text_temp = open(test_techo_file_path)
...
for i in range(len(test_techo_file)):
    ...
    pre_path = test_prediction_model_path +'/pre_model'+techo+'.dat'
    mdp.wr_vlb_pre_file(
    ...
    show_pic_dic_path = test_show_path +'/pre_model'+modify_model_dic['t_echo']
