...
compare_model_dic = get_real_pre_model_data('./model_li.dat')
...
for i in range(len(files_name)):
    ...
    for j in range(len(pre_files)):
        ...
        for k in range(len(pre_vp)):
            all_vel_unce.append((pre_vp_inter[k]-com_vp_inter[k])/ak_vp_inter[k])
            all_vel_unce_abs.append(np.abs((pre_vp[k]-com_vp[k])/ak_vp_inter[k]))

        all_all_vel_rmse_tmp = []
        for k in range(len(pre_vp)):
            all_all_vel_rmse_tmp.append(((pre_vp[k]-com_vp_inter[k])/ak_vp[k])**2)
        ...
        for k in range(len(pre_vp)):
            all_model_rmse_loss_tmp.append(((pre_vp[k]-com_vp_inter[k])/ak_vp[k]/0.03)**2)

        all_model_rmse_loss.append(np.sqrt(np.mean(all_model_rmse_loss_tmp)))



f = open('./get_err.txt','w')
